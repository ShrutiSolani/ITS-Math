...
mycursor.execute("select qid, topic from question")
data1 = mycursor.fetchall()
print(data1)
count = 0

fractions1_bp = Blueprint("fractions1_bp", __name__, template_folder="templates", static_folder="static")
log_object = Log()
...
def divide_with_whole():
    ...
    context = {"qid": qid, "que": que, "numerator": numerator, "hints": hints, "denominator": denominator,
               "ans_num": ans_num, "ans_den": ans_den, "rec_num": 1, "rec_den": div}
...
@fractions1_bp.route("/multiply-with-whole")
def multiply_with_whole():
    ...
    return render_template("division.html",
                           easy={"num": 2, "topic": "Multiple by Whole number", "ans_num": ans_num,
                                 "ans_den": ans_den, "q": q, "label1": "Numerator", "label2": "Denominator",
                                 "qid": qid})


@fractions1_bp.route("/add-fractions")
def add_fractions():
    global count
    global startTym
    startTym = datetime.datetime.now()
    ...
    for i in range(2):
        ...
        q.append(qs)
    q.insert(1, " ")
    contexts = {"num": 1, "qid": qid, "topic": "Add like fractions", "ans_num": ans_num, "ans_den": ans_den, "q": q,
...
@fractions1_bp.route("/unlike-add")
def unlike_add():
    global count
    global startTym
    startTym = datetime.datetime.now()
    ...
    answer = {"qid": qid, "que": que, "num_ans": ans_frac.numerator, "den_ans": ans_frac.denominator, "den": den_ans,
...
@fractions1_bp.route("/simplest-form")
def simplest_form():
    global count
    global startTym
    startTym = datetime.datetime.now()
...
@fractions1_bp.route("/normal-form")
def mixed_to_normal():
    global count
    global startTym
    startTym = datetime.datetime.now()
    ...
    answer = {"qid": qid, "que": que, "num_ans": frac.numerator, "den_ans": frac.denominator, "den": den,
              "num": num_ans}
    count += 1
    message = {"userid": session['userid'], "qid": qid, "qcount": count}
    log_object.log_entry(json.dumps(message))
...
@fractions1_bp.route("/mixed-fraction")
def question():
    global count
    global startTym
    startTym = datetime.datetime.now()
    ...
    answer = {"qid": qid, "que": que, "b0": box_ans[0], "b1": box_ans[1], "b2": box_ans[2], "b3": box_ans[3],
              "b4": box_ans[4]}
    count += 1
    message = {"userid": session['userid'], "qid": qid, "qcount": count}
    log_object.log_entry(json.dumps(message))
...
def compare():
    ...
    try:
        ...
        if fract1 > fract2:
            ...
            ans = "3"
        answer = {"qid": qid, "que": que, "lcm": lcm, "num1": num1, "den1": den1, "num2": num2, "den2": den2,
                  "ans": ans, "f1": f1, "f2": f2}
        count += 1
        message = {"userid": session['userid'], "qid": qid, "qcount": count}
        log_object.log_entry(json.dumps(message))
